...
void HAL_LCD_SpiInit(void)
{
    ...
    //fBitClock = fBRCLK/(UCBRx+1). (24Mhz/(UCBRX=1+1))
    EUSCI_B0->BRW = 1;
...
void lcd_draw_line_horizontal(
  ...
  uint16_t fColor
    ...
    // if larger than screen size, set a limit
    int i;
    uint16_t x1 = (len + x >= LCD_HORIZONTAL_MAX) ? LCD_HORIZONTAL_MAX : x + len;
    Crystalfontz128x128_SetDrawFrame(x, y, x1, y);
    HAL_LCD_writeCommand(CM_RAMWR);
    for(i = 0; i < x1 - x + 2; i++){ // +2 bc range includes x1 and x?
        HAL_LCD_writeData(fColor >> 8);
        HAL_LCD_writeData(fColor);
    }
...
void lcd_draw_line_vertical(
  ...
  uint16_t fColor
    ...
    int i;
    uint16_t y1 = (len + y >= LCD_VERTICAL_MAX) ? LCD_VERTICAL_MAX : y + len;
    Crystalfontz128x128_SetDrawFrame(x, y, x, y1);
    HAL_LCD_writeCommand(CM_RAMWR);
    for(i = 0; i < y1 - y + 2; i++){ // +2 bc range includes y1 and y
        HAL_LCD_writeData(fColor >> 8);
        HAL_LCD_writeData(fColor);
    }
}
